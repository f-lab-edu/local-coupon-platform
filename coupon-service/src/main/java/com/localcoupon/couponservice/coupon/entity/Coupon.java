...
public class Coupon extends BaseEntity {
  ...
  private Long storeId; //연관관계에서 id로 분리 후 조회 필요 시 Feign
  ...
  public Coupon update(CouponUpdateRequestDto request) {
    ...
    if (request.validPeriod() != null && request.validPeriod().getStart() != null
        && request.validPeriod().getEnd() != null) {
      this.validPeriod = new CouponPeriod(request.validPeriod().getStart(),
          request.validPeriod().getEnd());
    }
    if (request.issuePeriod() != null && request.issuePeriod().getStart() != null
        && request.issuePeriod().getEnd() != null) {
      this.issuePeriod = new CouponPeriod(request.issuePeriod().getStart(),
          request.issuePeriod().getEnd());
    }
    return this;
  }
}
