...
public class CouponPostProcessServiceImpl implements CouponPostProcessService {
    ...
    @Async
    public void sendQrCouponToUser(String userEmail, IssuedCoupon issuedCoupon) {
        try {
            // 1. QR 토큰 생성
            ...
            // 3. 이메일 발송
            couponMailService.sendCouponEmail(userEmail, issuedCoupon.getCoupon().getTitle(), qrImageUrl);

            // 4. 발급 쿠폰 업데이트
            issuedCouponRepository.save(issuedCoupon.postProcess(qrToken, qrImageUrl));

            log.info("[CouponPostProcessService] 후처리 완료 issuedCouponId={}, userId={}", issuedCoupon.getId(), userEmail);
        } catch (Exception e) {
            // TODO 실패 시 처리 방안
            log.error("[CouponPostProcessService] 후처리 실패 issuedCouponId={}, userId={}", issuedCoupon.getId(), userEmail, e);
        }
    }
}
